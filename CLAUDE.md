...
### 5. Key/Type targeting

`cu key` and `cu type` support `--app <name>` to target a specific app. When `--app` is specified, **AppleScript System Events** (activate + keystroke/key code) is used for reliable delivery. Without `--app`, CGEvent posts to the frontmost app.
...
- **JXA injection**: Never interpolate user input directly into JXA/AppleScript strings. Use `serde_json::to_string()` to escape, then assign to a JS variable.
- **AppleScript injection**: Escape `\` and `"` in user-provided text before embedding in AppleScript strings.

## Testing

- `tests/run.sh` — integration tests against live apps.
